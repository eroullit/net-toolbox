...
void help()
{
	...
	info("  -d|--dev <netdev>      Perform action on this NIC\n");
	info("  -b|--bpf <bpf file>    Berkeley Packet Filter configuration\n");
	info("  -p|--pcap <pcap file>  PCAP file to capture/replay/read\n");
	info("  -l|--cpu-list <list>   List of CPU to run on\n");
	info("  -c|--capture           Capture packet from the NIC\n");
	info("  -C|--compat-capture    Capture packet from the NIC in compatibility mode\n");
	info("  -r|--replay            Replay packet from PCAP on the NIC (not supported yet)\n");
	info("  -R|--read              Read PCAP and format it on stdout (not supported yet)\n");
	info("  -I|--info              Prints information about usable NICs\n");
	info("  -h|--help              Prints this help information\n");
	info("  -v|--version           Prints version information\n");
