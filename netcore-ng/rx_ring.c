...
struct rx_thread_ctx * rx_thread_create(const cpu_set_t run_on, const int sched_prio, const int sched_policy, const char * dev_name, const char * bpf_path, const char * pcap_path)
{
	int rc;
	...
	if ((rc = rx_nic_ctx_init(thread_config, dev_name)) != 0)
	{
		goto error;
	}
...
void rx_thread_destroy(struct rx_thread_ctx * thread_config)
{
	...
	rx_nic_ctx_destroy(&thread_config->nic);
	free(thread_config);
}
