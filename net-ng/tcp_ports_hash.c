...
int tcp_ports_hash_init(void)
{
	...
	if ((rc = hi_init_uint(&tcp_hash_handle, ARRAY_SIZE(ports_tcp))) != 0)
	{
		return (rc);
	}
	...
	{
		if ((rc = hi_insert_uint(tcp_hash_handle, ports_tcp[a].id, ports_tcp[a].port)) != 0)
		{
...
int tcp_ports_hash_search(const uint16_t tcp, const char ** port_name)
{
	...
	if (hi_get_uint(tcp_hash_handle, tcp, (void **)port_name) != 0)
	{
